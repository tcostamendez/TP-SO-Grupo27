...
void drawRectangle(uint32_t color, long long int width_pixels, long long int height_pixels, long long int initial_pos_x,
...
int32_t getRegisterSnapshot(int64_t *registers) { return sys_get_register_snapshot(registers); }

int32_t getCharacterWithoutDisplay(void) { return sys_get_character_without_display(); }

void *allocMemory(size_t size) { return sys_malloc(size); }

void freeMemory(void *ptr) { sys_free(ptr); }

void memoryStats(int *total, int *available, int *used) { sys_get_memory_stats(total, available, used); }

int createProcess(int argc, char **argv, void (*entry_point)(int, char **), int priority, int *targets,
				  int hasForeground) {
...
// Semaphore wrappers
// ======================
sem_t semOpen(const char *name, uint16_t initialValue) { return (sem_t)sys_sem_open(name, initialValue); }
