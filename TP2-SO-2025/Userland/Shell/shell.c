...
Command commands[] = {
	...
	{.name = "echo",
	 ...
	 .isBuiltIn = 0},
	{.name = "exit", .function = (int (*)(int, char **))_exit, .description = "Closes qemu window", .isBuiltIn = 1},
	{.name = "help",
	...
	{.name = "test_sync",
	 ...
	 .isBuiltIn = 0},
	{.name = "ps", .function = (int (*)(int, char **))entry_ps, .description = "Lists all processes", .isBuiltIn = 0},
	{.name = "loop",
	...
	{.name = "nice",
	 ...
	 .isBuiltIn = 0},
	{.name = "mem", .function = (int (*)(int, char **))entry_mem, .description = "Shows memory status", .isBuiltIn = 0},
	{.name = "wc",
...
static int parse_command(char *buffer, char *command, ParsedCommand *parsedCommand) {
	...
	while (arg != NULL && strcasecmp(arg, "|") != 0 && parsedCommand->argc < MAX_ARGUMENT_COUNT - 1) {
