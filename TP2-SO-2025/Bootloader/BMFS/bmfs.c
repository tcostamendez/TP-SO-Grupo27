...
void create(char *filename, unsigned long long maxsize) {
	...
	if (findfile(filename, &tempentry, &slot) == 0) {
		...
		for (tint = 0; tint < 64; tint++) {
			...
			{
				if (first_free_entry == -1) first_free_entry = tint;  // will use it for our new file
			}
		}
		...
		for (tint = 0; tint < num_used_entries + 1; tint++) {
			...
			if (tint < num_used_entries) prev_file_end = pEntry->StartingBlock + pEntry->ReservedBlocks;
		}
