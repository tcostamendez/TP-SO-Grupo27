...
int32_t syscallDispatcher(Registers *registers) {
	switch (registers->rax) {
		...
		case SYS_CREATE_PROCESS:
			return sys_create_process((int)registers->rdi, (char **)registers->rsi, (ProcessEntryPoint)registers->rdx,
									  (int)registers->rcx, (int *)registers->r8, (int)registers->r9);
		case SYS_GET_PID:
			return sys_get_pid();
...
int32_t sys_rectangle(uint32_t color, uint64_t width_pixels, uint64_t height_pixels, uint64_t initial_pos_x,
...
int32_t sys_exec(int32_t (*fnPtr)(void)) {
	...
	clearKeyFnMapNonKernel(map);  // avoid """processes/threads/apps""" registering keys across each
...
int32_t sys_get_character_without_display(void) { return getKeyboardCharacter(0); }
// ======================
...
int sys_create_process(int argc, char **argv, ProcessEntryPoint entryPoint, int priority, int targets[],
					   int hasForeground) {
...
void sys_modify_priority(int pid, int new_priority) { set_priority(pid, new_priority); }
...
int sys_get_process_info(ProcessInfo *info, int pid) { return get_process_info(info, pid); }
...
// Semaphore syscalls
// ======================
Sem sys_sem_open(const char *name, uint16_t value) { return semOpen(name, value); }
